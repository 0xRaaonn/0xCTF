...
``` text
<REDACTED — insert the flag revealed in the leaked system prompt>
```

> The provided screenshot confirms the system-prompt leak, but the
> portion containing the actual flag is not visible. Replace the
> placeholder above with the flag obtained from the challenge instance.
